...
class PythiaDemo:
    ...
    def _build_pythia_model(self):
        ...
        #model = torch.nn.DataParallel(Pythia(model_config))
        model.build()
...
#    init1 = int(db.reference("status/start).get())
    #question_text = db.reference("question").get()    
#    init = int(db.reference("status/mode").get())
    init2 = int(db.reference("status/confirm").get())
    init3 = (db.reference("answer").get())
    question_text = db.reference("question").get()
    if question_text != "":
#    if init == 5 and init2 == 0 and init3 == "null":
        start = time.time()
        image_text = "../send.jpg"
        ...
        if len(df.Prediction) != 0:
            ...
            if df.Confidence[0] < 55:
                output = "I don't know the answer to that"
        else:
            output = "I don't know the answer to that"
        print(df.Prediction)
        print(df.Confidence)
        print("pythia time:", time.time() - start)
        print(output)
#        cli.set("pythia-answer", output)
        ref = db.reference()
        ref.update({"question":""})
        ref.update({"answer":output})
#        ref.update({"status/confirm":0})
